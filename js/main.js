...
function getRandomPozitiveFloat (a, b, digits = 1) {
  let lower = Math.min(Math.abs(a), Math.abs(b));
  let upper = Math.max(Math.abs(a), Math.abs(b));
  result = (Math.random() * (upper - lower) + lower);
  return +result.toFixed(digits);
};

function getArrayNumberForImage (number=10) {
  let numberImage = Array.from( { length: number }, (v, i) => i + 1);
  numberImage = numberImage.map((item) => {
...
function generateArrayPhoto () {
  let ArrayPhoto = Array.from({length:20}, (x)=> x = photos)
  .flat()
...
function generateArrayObjects () {
 ...
 let numberAvatar = 0
  ...
  while (arrayObjects.length !== 10 ) {
    ...
    objects.locations = {lat: getRandomPozitiveFloat(lat.from, lat.before, lat.digits), //число с плавающей точкой — широта, случайное значение от 35.65000 до 35.70000.
                        lng: getRandomPozitiveFloat(lng.from, lng.before, lng.digits) //число с плавающей точкой — долгота, случайное значение от 139.70000 до 139.80000.
    }
    objects.offer = { title:title, //строка — заголовок предложения. Придумайте самостоятельно.
